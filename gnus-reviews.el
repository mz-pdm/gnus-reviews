            ...
            (save-excursion
              ...
              (let ((patch-end (save-excursion (org-end-of-subtree t t) (point)))
                ...
                (while (and (< current-comment-count comment-number)
                  ...
                  (cl-incf current-comment-count)
                  (when (= current-comment-count comment-number)
                    ;; Found the target comment, delete it
                    (let ((comment-start (progn
                                           ;; Look for quote block before status if present
                                           (save-excursion
                                             (beginning-of-line)
                                             (if (re-search-backward "^#\\+BEGIN_QUOTE$" patch-pos t)
                                                 (point)
                                               (progn (goto-char (match-beginning 0)) (point))))))
                          (comment-end (when (re-search-forward "^#\\+END_EXAMPLE$" patch-end t)
                                         (forward-line 1)
                                         (point))))
                ...
                (if deleted
                  ...
                  (message "Could not find comment %s to delete" comment-id))))))
        ;; Refresh the display buffer
        (gnus-reviews--refresh-comment-buffer)))))
    ...
    (when show-article-title
      (let ((article-title (gnus-reviews--with-org-buffer
                             (when-let ((patch-pos (gnus-reviews--find-org-node-by-id article-id)))
                               (save-excursion
                                 (goto-char patch-pos)
                                 (org-entry-get nil "ARTICLE_TITLE"))))))
        (when article-title
