...
(defcustom gnus-reviews-comment-max-length 1000
  "Maximum length of comment content to store."
  :type 'integer
  :group 'gnus-reviews)
...
(defvar gnus-reviews-patch-threads nil
  "Database of patch threads and their status.
Format: ((thread-id . (subject status patches reviews)) ...)")
  ...
  (when gnus-reviews-data-file
    (condition-case err
        (with-temp-file gnus-reviews-data-file
          ...
          (insert ")\n\n")
          (insert "(setq gnus-reviews-patch-threads\n")
          (prin1 gnus-reviews-patch-threads (current-buffer))
          (insert ")\n"))
      (error
    ...
    (condition-case err
      ...
      (error
       ...
       (setq gnus-reviews-comment-database nil
             gnus-reviews-patch-threads nil)))))
...
(defun gnus-reviews--extract-subject ()
  ...
  (let ((subject (gnus-with-article-buffer
    ...
    (when subject
      ...
      ;; Remove patch indicators
      (setq subject (replace-regexp-in-string "^\\[PATCH[^]]*\\]\\s*" "" subject))
      subject)))
